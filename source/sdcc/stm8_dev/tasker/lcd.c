...
/////////////////////////////////////////////////
//lcd_shiftFrameBuffer()
//move the contents of the frame buffer 1 pixel
//to the left (direction = 0), or right (direction = 1)
//the outside pixels are replaced with nothing
//direction - SHIFT_LEFT, SHIFT_RIGHT, wrap - 0 no wrap, 1 - wrap, update)
void lcd_shiftFrameBuffer(shiftDirection_t direction, uint8_t wrap, uint8_t update)
{
	uint8_t i, j, value, offset;
	uint16_t index = 0x00;
    ...
    if (direction == SHIFT_LEFT)
    {
	    ...
	    for (i = 0 ; i < LCD_NUM_PAGES ; i++)
	    {
            value = frameBuffer[index];             //store the value for wrap = 1
            ...
            if (wrap == 1)
                frameBuffer[index] = value;      //wrap?
            else
                frameBuffer[index] = 0x00;      //no wrap
    ...
    else if (direction == SHIFT_RIGHT)
    {
        ...
        for (i = 0 ; i < LCD_NUM_PAGES ; i++)
	    {
            ...
            value = frameBuffer[index + offset];             //store the value for wrap = 1
		    
		    for (j = 0 ; j < (LCD_WIDTH - 1) ; j++)
            {
                frameBuffer[index + offset] = frameBuffer[index + offset - 1];
                offset--;
            }

            if (wrap == 1)
                frameBuffer[index + offset] = value;      //wrap?
            else
                frameBuffer[index + offset] = 0x00;      //no wrap
...
void lcd_updateDisplay(void)
{
	...
	for (i = 0 ; i < LCD_NUM_PAGES ; i++)
	{
		lcd_setColumn(0);
		lcd_setPage(i);
		lcd_writeDataBurst(ptr, LCD_WIDTH);
		ptr += LCD_WIDTH;
	}	
}
...
void lcd_drawIcon(uint32_t xPosition, uint32_t yPosition, const ImageData *pImage, uint8_t update)
{
	...
	//set the pointer
	uint8_t *ptr = (uint8_t*)(pImage->pImageData);
	sizeX = pImage->xSize;
	sizeY = pImage->ySize;
...
void lcd_drawIconPage(uint8_t page, uint8_t offset, const ImageDataVertical *pImage)
{
    ...
    uint8_t i, j = 0;
    uint8_t* data = (uint8_t*)(pImage->pImageData);
...
////////////////////////////////////////////////
//Draw line into framebuffer, update the display
//if update = 1
void lcd_drawLine(int x0, int y0, int x1, int y1, uint8_t color, uint8_t update)
{
    ...
    for(;;)
    {
        //draw pixels into framebuffer - no update
        lcd_putPixelRam(x0, y0, color, 0);
